...
<style>
/* ═══════════════════════════════════════════════════════════════
   HISTÓRICO DE ATIVIDADE v2 — com Notificações no topo
═══════════════════════════════════════════════════════════════ */
...
.hist-resumo-icone {
    width: 44px; height: 44px; border-radius: var(--hist-radius-sm);
    display: flex; align-items: center; justify-content: center;
    font-size: 1.15rem; color: #fff; flex-shrink: 0;
    background: var(--card-cor);
}
.hist-resumo-info strong { display:block; font-size:1.5rem; font-weight:700; line-height:1; color:var(--hist-text); }
.hist-resumo-info span { font-size:.78rem; color:var(--hist-text-muted); font-weight:500; }
...
.hist-notif-item {
    ...
    gap: 0;
    border-radius: var(--hist-radius);
    border: 1px solid var(--notif-new-border);
    background: var(--notif-new-bg);
    box-shadow: 0 2px 8px rgba(245,158,11,.08);
    overflow: hidden;
    transition: background .25s, border-color .25s, opacity .3s, max-height .4s;
    max-height: 200px;
    position: relative;
}
.hist-notif-item.visto {
    background: var(--notif-seen-bg);
    border-color: var(--notif-seen-border);
    box-shadow: none;
    opacity: .65;
}
.hist-notif-item.saindo {
    max-height: 0;
    opacity: 0;
    margin: 0;
    padding: 0;
...
.hist-notif-barra {
    width: 100%;
    background: var(--notif-cor, #f59e0b);
    display: flex;
    align-items: center;
    justify-content: center;
    color: #fff;
...
.hist-notif-item.visto .hist-btn-visto {
    background: transparent;
    color: var(--hist-text-muted);
...
.hist-filtro-grupo label {
    font-size:.75rem; font-weight:600; color:var(--hist-text-muted);
    text-transform:uppercase; letter-spacing:.04em;
}
.hist-filtro-grupo input,
.hist-filtro-grupo select {
    padding:8px 11px;
    border:1px solid var(--hist-border);
    border-radius:var(--hist-radius-sm);
    background:var(--hist-bg);
...
.hist-item-icone-col {
    ...
    padding:16px 0 16px 18px; position:relative;
}
.hist-item-icone-col::after {
    content:''; position:absolute; top:50px; bottom:0; left:37px;
    width:2px; background:var(--hist-border);
}
...
.hist-badge-categoria {
    font-size:.68rem; font-weight:700; text-transform:uppercase;
    letter-spacing:.06em; padding:2px 8px; border-radius:20px; color:#fff;
}
.hist-item-usuario { font-size:.855rem; font-weight:600; color:var(--hist-text); }
.hist-item-tipo-util {
    font-size:.72rem; color:var(--hist-text-muted);
    background:var(--hist-bg); border:1px solid var(--hist-border);
...
.hist-item-meta span {
    font-size:.76rem; color:var(--hist-text-muted);
    display:flex; align-items:center; gap:3px;
}
.hist-campos-alterados { display:flex; flex-wrap:wrap; gap:4px; margin-top:5px; }
.hist-campo-tag {
    font-size:.7rem; background:rgba(245,158,11,.1); color:#92400e;
    border:1px solid rgba(245,158,11,.3); padding:2px 7px; border-radius:4px; font-weight:500;
}
body.dark-mode .hist-campo-tag { color:#fcd34d; background:rgba(245,158,11,.15); border-color:rgba(245,158,11,.35); }
.hist-item-data-col {
    padding:16px 18px 16px 8px; display:flex; flex-direction:column;
    align-items:flex-end; justify-content:center; gap:4px; min-width:138px;
}
...
.hist-paginacao {
    display:flex; align-items:center; justify-content:space-between;
    padding:14px 22px; border-top:1px solid var(--hist-border);
    flex-wrap:wrap; gap:8px; background:var(--hist-card-bg);
}
.hist-paginacao-info { font-size:.78rem; color:var(--hist-text-muted); }
.hist-paginacao-btns { display:flex; gap:4px; }
.hist-pag-btn {
    padding:5px 11px; border:1px solid var(--hist-border);
    border-radius:6px; background:var(--hist-card-bg); color:var(--hist-text);
...
.hist-spinner {
    ...
    border-radius:50%; animation:histSpin .6s linear infinite;
}
    ...
    <div class="hist-resumo-grid">
        <div class="hist-resumo-card" style="--card-cor:var(--cor-registro);" onclick="filtrarPorCategoria('registro')" data-categoria="registro">
            ...
            <div class="hist-resumo-info">
                <strong id="totalRegistos">{{ number_format($totalRegistos) }}</strong>
                <span>Registos</span>
            </div>
        </div>
        <div class="hist-resumo-card" style="--card-cor:var(--cor-consulta);" onclick="filtrarPorCategoria('consulta')" data-categoria="consulta">
            ...
            <div class="hist-resumo-info">
                <strong id="totalConsultas">{{ number_format($totalConsultas) }}</strong>
                <span>Consultas</span>
            </div>
        </div>
        <div class="hist-resumo-card" style="--card-cor:var(--cor-atualizacao);" onclick="filtrarPorCategoria('atualizacao')" data-categoria="atualizacao">
            ...
            <div class="hist-resumo-info">
                <strong id="totalAtualizacoes">{{ number_format($totalAtualizacoes) }}</strong>
        ...
        <div class="hist-resumo-card" style="--card-cor:var(--cor-pagamento);" onclick="filtrarPorCategoria('pagamento')" data-categoria="pagamento">
            ...
            <div class="hist-resumo-info">
                <strong id="totalPagamentos">{{ number_format($totalPagamentos) }}</strong>
...
<script>
// ══════════════════════════════════════════════════════════════════
// HISTÓRICO DE ATIVIDADE v2 — JavaScript
// ══════════════════════════════════════════════════════════════════
...
// Notificações vistas (persistido em localStorage)
let vistos = new Set(JSON.parse(localStorage.getItem(STOR_KEY) || '[]'));

// Mapas
const CORES  = { registro:'#3b82f6', consulta:'#8b5cf6', atualizacao:'#f59e0b', pagamento:'#10b981' };
const LABELS = { registro:'Registo', consulta:'Consulta', atualizacao:'Atualização', pagamento:'Pagamento' };
const ICONES = { registro:'fa-solid fa-user-plus', consulta:'fa-solid fa-stethoscope', atualizacao:'fa-solid fa-pen-to-square', pagamento:'fa-solid fa-credit-card' };
...
async function carregarNotificacoes() {
    try {
        const params = new URLSearchParams({ pagina:1, por_pagina:10 });
        const resp   = await fetch(`${HIST_API}?${params}`, {
            headers:{ 'X-CSRF-TOKEN':CSRF, 'Accept':'application/json' }
        });
        if (!resp.ok) return;
        const dados = await resp.json();
        dadosRecentes = dados.data || [];
...
function renderizarNotificacoes() {
    ...
    dadosRecentes.forEach(item => {
        ...
        const icone   = item.icone             || 'fa-solid fa-circle-info';
        ...
        li.innerHTML  = `
            ...
            <div class="hist-notif-corpo">
                ...
                <span class="hist-notif-desc" title="${escH(item.descricao || item.acao)}">${escH(item.descricao || item.acao)}</span>
...
function renderizarLista(dados) {
    ...
    dados.data.forEach(item => {
        ...
        const camposHTML = (item.campos_alterados && item.campos_alterados.length)
        ...
        <li class="hist-item">
            ...
            <div class="hist-item-corpo">
                ...
                <p class="hist-item-descricao">${escH(item.descricao || item.acao)}</p>
                ${camposHTML}
...
function renderizarPaginacao(dados) {
    ...
    btns.innerHTML = '';
    btns.appendChild(criarBtnPag('‹', dados.pagina_atual > 1, () => { estado.pagina--; carregarHistorico(); }));
    let pi = Math.max(1, dados.pagina_atual - 2);
    let pf = Math.min(dados.ultima_pagina, pi + 4);
    pi     = Math.max(1, pf - 4);
    for (let p = pi; p <= pf; p++) {
        const b = criarBtnPag(p, true, () => { estado.pagina = p; carregarHistorico(); });
        if (p === dados.pagina_atual) b.classList.add('ativo');
        btns.appendChild(b);
    }
    btns.appendChild(criarBtnPag('›', dados.pagina_atual < dados.ultima_pagina, () => { estado.pagina++; carregarHistorico(); }));
}
...
document.addEventListener('DOMContentLoaded', () => {
    // Ajustar max-height inicial do collapsible
    const col = document.getElementById('notifCollapsible');
    col.style.maxHeight = '9999px';

    carregarNotificacoes();
    carregarHistorico();

    // Auto-refresh das notificações a cada 60s
