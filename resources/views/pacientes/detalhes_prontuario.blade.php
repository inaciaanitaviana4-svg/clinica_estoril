...
@section('script')
    <script>
        ...
        async function obterConsulta() {
            const apiUrl = `{{ route('api_buscar_consultas_prontuario_medico', ['id_consulta' => ':id_consulta']) }}`
                .replace(':id_consulta', id_consulta);
            try {
        ...
        function renderDetail(consulta) {
            ...
            const diagHTML = consulta.diagnosticos.length ?
                consulta.diagnosticos.map(d =>
                    `<div class="diag-item"><div class="diag-bullet"></div><div>${d.descricao}</div></div>`).join('') :
