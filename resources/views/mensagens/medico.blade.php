...
.av { border-radius:50%; object-fit:cover; flex-shrink:0; display:block; }
.av-fallback { border-radius:50%; display:flex; align-items:center; justify-content:center; font-weight:700; color:#fff; flex-shrink:0; }
...
.msg-item-data{font-size:11px;color:var(--msg-sub);display:flex;align-items:center;gap:6px;}
...
@media(max-width:768px){:root{--sidebar-w:100%;}
...
$nome_eu = session('nome_utilizador') ?? 'Médico';
@endphp
...
<div class="msg-wrap">
    ...
    <div class="msg-sidebar">
        <div class="msg-sidebar-head">
            ...
            <div class="msg-search">
                ...
                <input type="text" placeholder="Pesquisar paciente ou consulta…" oninput="filtrarConsultas(this.value)">
            </div>
        </div>
        ...
        <div class="msg-list">
            ...
            <a href="?consulta={{ $c->id_consulta }}"
               ...
               data-search="{{ strtolower($c->nome_paciente . ' ' . $c->tipo_consulta . ' ' . $c->servico) }}">
                <div class="msg-item-top">
                    {!! avatarHtml($c->nome_paciente, $c->foto_paciente ?? null, 38, 'verde') !!}
                    <div class="msg-item-info">
                        <div class="msg-item-nome">{{ $c->nome_paciente }}</div>
                        <div class="msg-item-esp">{{ $c->tipo_consulta ?? $c->servico ?? '—' }}</div>
                    </div>
                    @if($c->nao_lidas > 0)
            ...
            <div class="msg-empty-sidebar">
                ...
                <p>Sem pacientes disponíveis.<br>As mensagens ficam disponíveis após consultas concluídas.</p>
            </div>
            @endforelse
        </div>
    </div>
    ...
    <div class="msg-main">
        ...
        @if($consulta_sel && $id_util_paciente)

            {{-- Header --}}
            <div class="msg-chat-header">
                {!! avatarHtml($consulta_sel->nome_paciente, $consulta_sel->foto_paciente ?? null, 44, 'verde') !!}
                <div class="msg-chat-header-info">
                    <p class="msg-chat-nome">{{ $consulta_sel->nome_paciente }}</p>
                    <p class="msg-chat-sub">Paciente · Consulta N˚{{ $consulta_sel->id_consulta }}</p>
                </div>
            ...
            <div class="msg-bubbles" id="bubbles">
                ...
                @forelse($mensagens as $msg)
                    @php
                        ...
                        $fotoB   = $minha ? ($foto_eu ?? null) : ($consulta_sel->foto_paciente ?? null);
                        $nomeB   = $minha ? $nome_eu : $consulta_sel->nome_paciente;
                        $corB    = $minha ? 'azul' : 'verde';
                    @endphp
                ...
                @empty
                    <div style="text-align:center;padding:40px;color:var(--msg-sub);font-size:13px;">
                        ...
                        Ainda sem mensagens. Inicie a conversa com o paciente!
                    </div>
                @endforelse
            ...
            <div class="msg-input-area">
                <div class="msg-input-box">
                    <textarea id="msgTexto" rows="1"
                              placeholder="Escreva a sua mensagem ao paciente…"
                ...
                <p style="font-size:11px;color:var(--msg-sub);margin:5px 0 0;text-align:center;">
                    ...
                    Comunicação clínica protegida · visível apenas entre médico e paciente
                </p>
            </div>
        ...
        @else
            <div class="msg-placeholder">
                ...
                <p>Escolha um paciente à esquerda para<br>ver ou iniciar uma conversa.</p>
            </div>
        @endif
    </div>
</div>
...
const ID_INTERLOCUTOR = {{ $id_util_paciente ?? 'null' }};
const ID_CONSULTA     = {{ $consulta_sel->id_consulta ?? 'null' }};
const CSRF            = document.querySelector('meta[name=csrf-token]')?.content ?? '';

const FOTO_EU       = @json($foto_eu ?? null);
const FOTO_PACIENTE = @json($consulta_sel->foto_paciente ?? null);
const NOME_EU       = @json($nome_eu);
const NOME_PACIENTE = @json($consulta_sel->nome_paciente ?? '');
...
function avatarEl(nome, foto, size, cor) {
    if (foto) {
        ...
        img.onerror = function() { this.replaceWith(criarFallback(nome.charAt(0).toUpperCase(), size, cor)); };
        return img;
    }
...
function adicionarBolha({id_mensagem, conteudo, hora, minha}) {
    ...
    const foto = minha ? FOTO_EU : FOTO_PACIENTE;
    const nome = minha ? NOME_EU : NOME_PACIENTE;
    const cor  = minha ? 'azul' : 'verde';
